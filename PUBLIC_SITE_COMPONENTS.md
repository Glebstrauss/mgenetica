...
The public site uses editorial components to explain the MGenetica learning path, organize modules, support discovery and present selective consulting/training services. These patterns should remain scientific, premium, clear and modular enough for future content management through the internal app.
...
- Service pages: consulting and training pages use `.consulting-hero`, `.consulting-fit`, `.consulting-problem-map`, `.consulting-packages`, `.consulting-method`, `.consulting-readiness`, `.consulting-triage`, `.consulting-boundary`, `.consulting-next-step`, `.training-hero`, `.training-fit`, `.training-formats`, `.training-themes`, `.training-method`, `.training-deliverables`, `.training-proposal`, `.training-boundary` and `.training-next-step` to separate education from paid application without adding backend assumptions.

## Component Families

- Hero components: `.hero`, `.hero-panel-proof`, `.page-hero`, `.profile-hero`, `.consulting-hero` and `.training-hero` introduce the page and should not coexist with Quarto's automatic title block.
...
- Learning-path components: `.hero-learning-path`, `.hero-signal`, `.hero-action-note`, `.hero-action-note-hint`, `.hero-action-note-grid`, `.hero-action-note-item`, `.home-trust-anchors`, `.home-first-session`, `.home-output-standard`, `.home-application-path`, `.home-application-path-grid`, `.home-application-path-item`, `.home-path-contract`, `.home-orientation`, `.home-audience`, `.home-start-criteria`, `.home-returning`, `.module-anatomy-grid`, `.entry-decision`, `.home-evidence`, `.home-outcome-map`, `.home-readiness`, `.home-continuity`, `.resource-grid`, `.resource-card`, `.learning-loop-grid`, `.modules-completion-flow`, `.modules-output-route`, `.modules-evidence-standard`, `.modules-certificate-route`, `.modules-next-step`, `.route-overview`, `.route-week-decision`, `.route-start-today`, `.route-recovery-plan`, `.route-session-plan`, `.route-output-check`, `.route-session-split`, `.route-evidence-ladder`, `.route-phase-handoff`, `.route-weekly-output`, `.route-table-guide`, `.route-map-intro`, `.route-finish-band`, `.module-reading-rhythm`, `.module-session-plan`, `.module-technical-scan`, `.module-script-lab`, `.module-evidence-path`, `.module-practice-contract`, `.module-study-checkpoint`, `.module-takeaways`, `.module-after-quiz`, `.module-close-check`, `.module-return-note`, `.module-phase-start` and `.module-phase-note` clarify progression without turning the site into a dashboard.
- Service components: `.consulting-fit`, `.consulting-problem-map`, `.consulting-packages`, `.consulting-method`, `.consulting-readiness`, `.consulting-triage`, `.consulting-boundary`, `.consulting-next-step`, `.training-fit`, `.training-formats`, `.training-themes`, `.training-method`, `.training-deliverables`, `.training-proposal`, `.training-boundary` and `.training-next-step` keep commercial pages structured as diagnostic decision routes, not sales landing pages.
