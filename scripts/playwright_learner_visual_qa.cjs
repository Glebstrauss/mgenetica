...
(async () => {
  ...
  for (const viewport of viewports) {
    ...
    await assertBoxBetween(page, '.brand-logo img', `${viewport.name} header logo`, viewport.name === 'mobile' ? 76 : 88, 92, viewport.name === 'mobile' ? 76 : 88, 92)
    await assertBoxBetween(page, '.hero-proof-mark img', `${viewport.name} proof logo`, viewport.name === 'mobile' ? 48 : 56, 60, viewport.name === 'mobile' ? 48 : 56, 60)
    await assertBoxBetween(page, '.app-header', `${viewport.name} header`, 300, viewport.width, 110, viewport.name === 'mobile' ? 300 : 132)
