...
(async () => {
  ...
  for (const viewport of viewports) {
    ...
    await assertBoxBetween(page, '.brand-logo img', `${viewport.name} header logo`, viewport.name === 'mobile' ? 64 : 72, 76, viewport.name === 'mobile' ? 64 : 72, 76)
    await assertBoxBetween(page, '.hero-proof-mark img', `${viewport.name} proof logo`, viewport.name === 'mobile' ? 36 : 40, 44, viewport.name === 'mobile' ? 36 : 40, 44)
    await assertBoxBetween(page, '.app-header', `${viewport.name} header`, 300, viewport.width, 94, viewport.name === 'mobile' ? 280 : 116)
